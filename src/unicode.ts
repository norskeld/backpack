...
export class UnicodeCodec {
  private static readonly cache: Map<string, Uint8Array> = new Map()

  static encode(string: string): Uint8Array {
    if (UnicodeCodec.cache.has(string)) {
      return UnicodeCodec.cache.get(string)
    }
    ...
    UnicodeCodec.cache.set(string, bytes)
